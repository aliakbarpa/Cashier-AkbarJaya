...
Generates sales reports with date range filtering
"""
...
def generate_sales_report(parent):
    ...
    def generate():
        ...
        for idx, row in df_filtered.iterrows():
            # Handle empty or invalid products column
        ...
        report_text += f"From {start_date} to {end_date}\n\n"
        report_text += f"👥 Cashiers: {', '.join(cashiers)}\n"
        report_text += f"🛍️  Customers: {', '.join(customers)}\n\n"

        report_text += f"{'Product ID':10} {'Qty Sold':>10} {'Revenue($)':>12} {'Stock Left':>12}\n"
        report_text += "-"*50 + "\n"
        
        for pid, data in product_summary.items():
            # Safe access to stock information
            if pid in df_products.index:
                try:
